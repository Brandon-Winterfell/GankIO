...
import com.huahua.gankio.bean.QianduanBean;
...
public class fragmentQianDuan extends Fragment
    ...
    private List<QianduanBean> mQianduanBeanList = new ArrayList<>();
    ...
    public List<String> loadArray(Context context) {
        SharedPreferences spf = context.getSharedPreferences("qianduanSharedPreferences", Context.MODE_PRIVATE);
        mOldSelected.clear();
        int size = spf.getInt("qianduan_Status_size", 0);

        for (int i = 0; i < size; i++) {
            mOldSelected.add(spf.getString("qianduan_Status_" + i, null));
        }
    ...
    public void saveArray(Context context) {
        ...
        editor.putInt("qianduan_Status_size", newSize);

        for (int i = 0; i < newSize; i++) {
            editor.putString("qianduan_Status_" + i, newSelected.get(i));
        }
    ...
    private void initView(View view) {
        ...
        mSwipeRefreshLayout = (SwipeRefreshLayout) view.findViewById(R.id.swipe_refresh_item);
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE
                    ...
                    OkhttpManager.getAsync("http://gank.io/api/data/前端/15/" + (++page), new IDataCallBack() {
                        @Override
                        ...
                        @Override
                        public void requestSuccess(String result) { // here is ui thread. can refresh ui
                            isLoadingMore = false;
                            mSwipeRefreshLayout.setRefreshing(false);
                            ...
                            if (!TextUtils.isEmpty(result)) {
                                ...
                                if (parseJSON(result) != null) {
                                    // mQianduanBeanList.clear(); // 下拉刷新才要清空数据
                                    mQianduanBeanList.addAll(parseJSON(result));
    ...
    public void onRefresh() {
        // 下拉刷新
        page = 1;
        Log.i("text", "fragmentQianDuan我下拉了");

        // 设置下拉状态
        mSwipeRefreshLayout.setRefreshing(true);

        OkhttpManager.getAsync("http://gank.io/api/data/前端/15/1", new IDataCallBack() {
            @Override
            public void requestFailure(Request request, IOException e) {
                // 这里的操作都将会切换到主线程中执行
                mSwipeRefreshLayout.setRefreshing(false);
    ...
    private List<QianduanBean> parseJSON(String jsonString) {
        // 其实可以return一个list 然后addall的 传入泛型
        // 好像不可以 因为对应着属性
        List<QianduanBean> newData = new ArrayList<QianduanBean>();
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                QianduanBean qianduanBean = new QianduanBean(id, url, desc, publishedAt, who);
                newData.add(qianduanBean);
            }
    ...
    class MyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            if (holder instanceof MyViewHolder) {
                ...
                QianduanBean qianduanBean = mQianduanBeanList.get(position);
                myViewHolder.tvTitle.setText(qianduanBean.getDesc());
                myViewHolder.tvAuthor.setText(qianduanBean.getWho());
                myViewHolder.tvDate.setText(qianduanBean.getPublishedAt());

                if (newSelected.contains(qianduanBean.getId())) {
                ...
                FootEmptyViewHolder fevh = (FootEmptyViewHolder)holder; // 空的布局
            }
        ...
        public int getItemCount() { // 那你添加一个空的1像素的顶部布局 not 1dp
            return mQianduanBeanList.size() + 1;  // 添加一个底部布局
        }
        ...
        public void onClick(View v) {
            ...
            QianduanBean qianduanBean = mQianduanBeanList.get(position);
            String url = qianduanBean.getUrl();

            // 保存id
            newSelected.add(qianduanBean.getId());

            // 跳转到webview的activity
            Intent intent = new Intent(fragmentQianDuan.this.getActivity(), ActivityWebView.class);
            intent.putExtra("url", url);
            fragmentQianDuan.this.getActivity().startActivity(intent);
        }
