...
import com.huahua.gankio.bean.iOSBean;
...
public class fragmentIOS extends Fragment
    ...
    private List<iOSBean> miOSBeanList = new ArrayList<>();
    ...
    public List<String> loadArray(Context context) {
        SharedPreferences spf = context.getSharedPreferences("iosSharedPreferences", Context.MODE_PRIVATE);
        mOldSelected.clear();
        int size = spf.getInt("ios_Status_size", 0);

        for (int i = 0; i < size; i++) {
            mOldSelected.add(spf.getString("ios_Status_" + i, null));
        }
    ...
    public void saveArray(Context context) {
        ...
        editor.putInt("ios_Status_size", newSize);

        for (int i = 0; i < newSize; i++) {
            editor.putString("ios_Status_" + i, newSelected.get(i));
        }
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE && lastVisibleItem + 2 >= mLinearLayoutManager.getItemCount()) {
                    ...
                    OkhttpManager.getAsync("http://gank.io/api/data/iOS/15/" + (++page), new IDataCallBack() {
                        @Override
                        ...
                        public void requestSuccess(String result) {
                            ...
                            if (!TextUtils.isEmpty(result)) {
                                ...
                                if (parseJSON(result) != null) {
                                   // miOSBeanList.clear(); // 下拉刷新才要清空数据
                                    miOSBeanList.addAll(parseJSON(result));
    ...
    public void onRefresh() {
        // 下拉刷新
        page = 1;
        Log.i("text", "fragmentIOS我下拉了");

        // 设置下拉状态
        mSwipeRefreshLayout.setRefreshing(true);

        OkhttpManager.getAsync("http://gank.io/api/data/iOS/15/1", new IDataCallBack() {
            @Override
            public void requestFailure(Request request, IOException e) {
                // 这里的操作都将会切换到主线程中执行
                mSwipeRefreshLayout.setRefreshing(false);
    ...
    private List<iOSBean> parseJSON(String jsonString) {
        // 其实可以return一个list 然后addall的 传入泛型
        // 好像不可以 因为对应着属性
        List<iOSBean> newData = new ArrayList<iOSBean>();
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                iOSBean iosBean = new iOSBean(id, url, desc, publishedAt, who);
                newData.add(iosBean);
            }
    ...
    class MyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            if (holder instanceof MyViewHolder) {
                ...
                iOSBean iosBean = miOSBeanList.get(position);
                myViewHolder.tvTitle.setText(iosBean.getDesc());
                myViewHolder.tvAuthor.setText(iosBean.getWho());
                myViewHolder.tvDate.setText(iosBean.getPublishedAt());

                if (newSelected.contains(iosBean.getId())) {
                ...
                FootEmptyViewHolder fevh = (FootEmptyViewHolder)holder; // 设为透明
            }
        ...
        public int getItemCount() { // 那你添加一个空的1像素的顶部布局 not 1dp
            return miOSBeanList.size() + 1;  // 添加一个底部布局
        }
        ...
        public void onClick(View v) {
            ...
            iOSBean iOSBean = miOSBeanList.get(position);
            String url = iOSBean.getUrl();

            // 保存id
            newSelected.add(iOSBean.getId());

            // 跳转到webview的activity
            Intent intent = new Intent(fragmentIOS.this.getActivity(), ActivityWebView.class);
            intent.putExtra("url", url);
            fragmentIOS.this.getActivity().startActivity(intent);
        }
